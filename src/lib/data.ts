...
export async function readBillingData(): Promise<BillingData> {
  try {
    const content = await fs.readFile(BILLING_PATH, 'utf-8');
    return JSON.parse(content) as BillingData;
  } catch (error) {
    if (error instanceof Error && 'code' in error && (error as NodeJS.ErrnoException).code === 'ENOENT') {
      ...
      await writeBillingData(initialData);
      return initialData;
    }
    console.error('Error reading billing.json:', error);
    throw new Error('Failed to read billing data');
  }
}

export async function writeBillingData(data: BillingData): Promise<void> {
  try {
    const content = JSON.stringify(data, null, 2);
    await fs.writeFile(BILLING_PATH, content, 'utf-8');
  } catch (error) {
    console.error('Error writing billing.json:', error);
    throw new Error('Failed to write billing data');
  }
}
